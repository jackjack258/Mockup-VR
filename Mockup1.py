...
def update_progress():
    global progressBar, sphere, tex
    while True:
        if viz.cluster.getClient() == viz.MASTER:
            if isLAN:
                response = requests.get(url='http://127.0.0.1:7860/sdapi/v1/progress')
                yield viztask.waitTime(1)
            else:
                
                response = requests.post(url='http://192.168.99.14:7860/forward_progress', json=payload)
...
def onInpaintSubmit():
    ...
    global_steps = int(numInpaintStepsBox.get())
    global_prompt = create_prompt(inpaintPromptBox.get(), lora_name)
    
    

global_data = None
def sendInpaintAPIrequest(prompt, steps):
    global width, height, isLAN, sphere, data
...
def onSubmit(button, state):
    global seed, txt2imgGUI, isInpaintMode, global_prompt, global_steps, global_hires_fix, lora_name
    if button == saveButton and state == viz.DOWN:
        global_steps = int(numStepsBox.get())
        global_hires_fix = bool(hires.get())
        global_prompt = create_prompt(promptBox.get(), lora_name)
...
def onReturn(button):
    ...
    if viz.key.isDown(viz.KEY_RETURN):
        ...
        if isInpaintMode:
            ...
            threading.Thread(target=sendAPIrequest, args=(global_prompt, global_steps, global_hires_fix)).start()
...
viz.callback(viz.KEYDOWN_EVENT, onReturn)
...
if isCAVE:
    with viz.cluster.MaskedContext(viz.MASTER):
        ...
        txt2imgGUI.addSeparator(padding=(20, 20))
        saveButton = txt2imgGUI.addItem(viz.addButtonLabel('Save Settings, enter to process'), align=viz.ALIGN_RIGHT_CENTER)
        inpaintButton = txt2imgGUI.addItem(viz.addButtonLabel('Inpaint Mode'), align=viz.ALIGN_RIGHT_CENTER)
        vizact.onbuttondown(saveButton, lambda: onSubmit(saveButton, viz.DOWN))
...
#vizact.onkeydown(viz.KEY_RETURN, lambda: onReturn(viz.KEY_RETURN))
